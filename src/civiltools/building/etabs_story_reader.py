"""
ETABS Story Reader for Live Load Management

This module provides functionality to read story/floor information directly from an
active ETABS model and create corresponding Floor objects for live load management.

It serves as a bridge between ETABS and the live load management system, extracting
story data and converting it into the internal Floor/Area structure.
...
Usage:
    ...
    # Read stories from the model
    reader = ETABSStoryReader(etabs_conn.etabs)
    floors = reader.get_floors()
    
    # Use with live load database
    project = reader.create_project(project_id="my_project", project_name="My Building")
"""
...
class ETABSStoryReader:
    ...
    Attributes:
        etabs: The EtabsModel instance from etabs_api
        story_obj: The Story helper object for accessing story data
        database: Optional LiveLoadDatabase instance for validation
    """

    def __init__(self, etabs_obj, database: Optional[LiveLoadDatabase] = None):
        """
    ...
    def get_all_stories_info(self) -> dict[str, dict]:
        """
    ...
    def create_floors_from_stories(self) -> dict[str, Floor]:
        ...
        Note:
            - Areas within each floor are NOT automatically imported
              (that requires geometry/structural modeling integration)
            - Users must assign use types via floor.apply_default_use()
              or area.set_use_type()
        """
        floors = {}
    ...
    def create_project(
        ...
        Example:
            >>> project = reader.create_project(
            ...     project_id="bldg_001",
            ...     project_name="Office Building",
            ...     default_floor_use="office_general"
            ... )
        """
        if not project_name:
            try:
    ...
    def get_story_level_difference(self, story1: str, story2: str) -> float:
        """
        Calculate the vertical distance between two stories.
        
        Args:
            story1: First story name.
            story2: Second story name.
        
        Returns:
            Elevation difference (story1 - story2).
        """
        elev1 = self.get_story_elevation(story1)
        elev2 = self.get_story_elevation(story2)
        return elev1 - elev2
...
class ETABSProjectBuilder:
    ...
    Example:
        ...
        >>> project = (builder
        ...     .set_project_info("office_001", "Main Office Building")
        ...     .apply_default_use_to_all("office_general")
        ...     .apply_use_to_floor("Story1", "office_waiting_ground")
        ...     .build())
    """
